[index.html (2).html](https://github.com/user-attachments/files/24119337/index.html.2.html)
<!DOCTYPE html>
<html lang="zh-TW">
<head>
    <meta charset="UTF-8">
    ...
    <style>
        ...
        .timeline-dot {
            width: 12px; height: 12px; background-color: #d6d3d1;
            border-radius: 50%; position: absolute; left: -6px; top: 6px;
            border: 2px solid #f5f5f0;
        }
        .timeline-dot.important {
            background-color: #8E354A; width: 16px; height: 16px; left: -8px;
        }
...
<body>
    ...
    <!-- Header -->
    <header class="fixed top-0 left-0 w-full bg-muji-bg/90 backdrop-blur-md z-50 px-6 py-4 border-b border-stone-200 shadow-sm transition-all duration-300">
    ...
    <main class="mt-20 px-4 max-w-md mx-auto">
        ...
        <section id="plan" class="tab-content active">
            <div class="flex overflow-x-auto space-x-3 pb-4 no-scrollbar mb-2" id="day-selector">
                <!-- Javascript will inject days -->
            </div>
            <div id="itinerary-list" class="space-y-6 ml-2 border-l-2 border-stone-300 pl-6 relative">
                <!-- Javascript will inject timeline -->
            </div>
        </section>

        <!-- ================= GUIDE MODULE ================= -->
        <section id="guide" class="tab-content">
            <h2 class="text-2xl font-serif font-bold mb-4 text-muji-text">深度導覽</h2>
            
            <!-- Category Tabs -->
            <div class="flex space-x-4 mb-6 border-b border-stone-200 pb-2">
                <button onclick="filterGuide('sight')" class="text-sm font-bold text-muji-text border-b-2 border-muji-accent pb-2">景點</button>
                <button onclick="filterGuide('shop')" class="text-sm text-stone-400 hover:text-muji-text pb-2">逛街購物</button>
            </div>
        ...
        <section id="wallet" class="tab-content">
            ...
            <div class="bg-muji-card rounded-xl p-5 shadow-sm mb-6">
                ...
                <div class="bg-stone-100 rounded-lg p-3 mb-3 text-right">
                    ...
                    <input type="text" id="calc-input" placeholder="輸入算式..." class="w-full bg-transparent text-right text-2xl font-bold text-muji-text outline-none placeholder-stone-300">
            ...
            <div class="bg-muji-card rounded-xl p-5 shadow-sm">
                ...
                <div class="flex space-x-2 mb-3">
                    <input type="text" id="expense-item" placeholder="品項" class="flex-1 bg-stone-50 rounded px-3 py-2 outline-none">
                    <input type="number" id="expense-amount" placeholder="日幣" class="w-24 bg-stone-50 rounded px-3 py-2 outline-none">
                </div>
                <div class="flex justify-between items-center mb-4">
                    <label class="flex items-center space-x-2 text-stone-500 text-sm cursor-pointer bg-stone-100 px-3 py-2 rounded-lg">
                        <i class="fas fa-camera"></i> <span>照片</span>
                        <input type="file" id="expense-photo" accept="image/*" capture="environment" class="hidden" onchange="previewImage()">
                    </label>
                    <button onclick="addExpense()" class="bg-muji-text text-white px-6 py-2 rounded-lg shadow">新增</button>
                </div>
                ...
                <div class="space-y-3 max-h-60 overflow-y-auto pr-1" id="expense-list"></div>
                ...
                <button onclick="clearExpenses()" class="w-full mt-4 text-xs text-red-300 hover:text-red-500">清空資料</button>
            </div>
        </section>
        ...
        <section id="lists" class="tab-content">
            ...
            <div class="bg-muji-card rounded-xl p-5 shadow-sm">
                ...
                <textarea id="memo-area" rows="8" class="w-full bg-stone-50 rounded-lg p-3 text-sm outline-none resize-none" placeholder="輸入筆記，網址會自動轉連結..."></textarea>
        ...
        <section id="info" class="tab-content">
            ...
            <div class="grid gap-4">
                ...
                <div class="bg-gradient-to-br from-blue-50 to-white p-5 rounded-xl shadow-sm border border-blue-100 relative overflow-hidden">
                    <div class="flex justify-between items-start z-10 relative">
                        <div>
                            <div class="text-xs text-stone-400 mb-1 flex items-center">
                                <i class="fas fa-location-dot mr-1"></i> 岡山 (Okayama)
                            </div>
                            <div class="text-4xl font-bold text-muji-text flex items-start">
                                <span id="weather-temp">--</span><span class="text-lg mt-1">°C</span>
                            </div>
                            <div class="text-sm text-stone-600 mt-1" id="weather-desc">載入中...</div>
                        </div>
                        <div class="text-5xl text-blue-300 weather-icon-anim" id="weather-icon">
                            <i class="fas fa-cloud"></i>
                        </div>
                    </div>
                    
                    <!-- Outfit Advice -->
                    <div class="mt-4 pt-3 border-t border-blue-100 z-10 relative">
                        <div class="text-xs font-bold text-muji-sub mb-1"><i class="fas fa-tshirt mr-1"></i>穿衣建議</div>
                        <p class="text-sm text-stone-600" id="weather-advice">正在分析氣溫...</p>
                    </div>
                    
                    <!-- Fallback Link -->
                    <a href="https://www.jma.go.jp/bosai/forecast/" target="_blank" class="absolute bottom-2 right-3 text-[10px] text-blue-300 hover:text-blue-500">日本氣象廳 ></a>
                </div>
                ...
                <div class="bg-white p-5 rounded-xl shadow-sm">
                    ...
                    <div class="space-y-3">
                        ...
                        <div class="flex justify-between items-center">
                            <span class="text-stone-600"><i class="fas fa-globe mr-2 w-5"></i>台北駐大阪辦事處</span>
                            <a href="tel:+81-6-6227-8623" class="text-muji-accent font-bold">06-6227-8623</a>
                        </div>
                    </div>
                </div>
                ...
                <!-- Tips -->
                <div class="bg-white p-5 rounded-xl shadow-sm">
                    <h3 class="font-bold text-muji-text mb-3 border-b pb-2">旅遊小須知</h3>
                    <ul class="list-disc list-inside text-sm text-stone-600 space-y-2">
                        <li>日本電壓 100V，插座為雙平腳（與台灣相同）。</li>
                        <li>岡山/廣島手扶梯多為靠左站立。</li>
                        <li>租車自駕務必攜帶台灣駕照 + 日文譯本。</li>
                        <li>同一天同一店消費滿 5000 日圓 (未稅) 可退稅，需出示護照。</li>
                        <li>公共場所請勿邊走邊吃。</li>
                    </ul>
                </div>
            </div>
    ...
    <script>
        ...
        const itinerary = {
            "Day 1": {
                ...
                events: [
                    ...
                    { time: "16:00", title: "飯店 Check-in", desc: "Daiwa Roynet Hotel Okayama Station", type: "hotel", icon: "fa-bed", link: "https://www.google.com/maps/search/?api=1&query=Daiwa+Roynet+Hotel+Okayama+Station" },
                    { time: "17:00", title: "AEON Mall 逛街", desc: "無印良品、大創、UNIQLO", type: "activity", icon: "fa-shopping-bag", link: "https://www.google.com/maps/search/?api=1&query=AEON+MALL+Okayama" },
            ...
            "Day 2": {
                ...
                events: [
                    ...
                    { time: "13:30", title: "千光寺、貓之細道", desc: "纜車單程 500日圓，走路下山", type: "activity", icon: "fa-cat", link: "https://www.google.com/maps/search/?api=1&query=Senkoji+Temple+Onomichi" },
                    { time: "18:00", title: "尾道國際飯店", desc: "自駕前往，附近有超市", type: "hotel", icon: "fa-bed", link: "https://www.google.com/maps/search/?api=1&query=Onomichi+Kokusai+Hotel" }
                ]
            },
            "Day 3": {
                date: "12/30 (二)",
        ...
        const guides = [
            {
                ...
                tag: "世界遺產",
                mapLink: "https://www.google.com/maps/search/?api=1&query=Itsukushima+Jinju"
            },
            {
                type: 'sight',
                title: "倉敷美觀地區",
                subtitle: "江戶時代的白壁倉庫群",
                content: "1642年成為幕府直轄地，因運送米糧而繁榮。保留了白壁土藏建築與柳樹成蔭的倉敷川，充滿懷舊氛圍。夜間點燈後更有另一番風味，適合穿著和服漫步。",
                tag: "歷史街道",
                mapLink: "https://www.google.com/maps/search/?api=1&query=Kurashiki+Bikan+Historical+Quarter"
            },
            {
                type: 'sight',
                title: "千光寺與貓之細道",
                ...
                tag: "攝影熱點",
                mapLink: "https://www.google.com/maps/search/?api=1&query=Senkoji+Temple+Onomichi"
            },
            {
                type: 'sight',
                title: "岡山後樂園",
                ...
                tag: "庭園",
                mapLink: "https://www.google.com/maps/search/?api=1&query=Okayama+Korakuen"
            },
            // Shopping Data Added
            {
                type: 'shop',
                title: "AEON Mall 岡山",
                subtitle: "西日本旗艦級購物中心",
                content: "位於岡山車站旁步行5分鐘，擁有約350間店鋪。從國際品牌到日系雜貨應有盡有，更有大型超市可購買伴手禮與水果。內有高島屋美食街，非常方便。",
                tag: "綜合商場",
                mapLink: "https://www.google.com/maps/search/?api=1&query=AEON+MALL+Okayama"
            },
            {
                type: 'shop',
                title: "岡山一番街",
                subtitle: "車站地下購物天堂",
                content: "位於JR岡山站地下，與AEON Mall連通。主打年輕女性時尚服飾、雜貨以及化妝品，雨天也能輕鬆逛街。也有許多受歡迎的甜點店進駐。",
                tag: "地下街",
                mapLink: "https://www.google.com/maps/search/?api=1&query=Okayama+Ichibangai"
            },
            {
                type: 'shop',
                title: "Sun Station 岡山",
                subtitle: "伴手禮與便當的一級戰區",
                content: "位於車站建築內，是購買岡山特產（吉備糰子、白桃果凍）的最佳地點。若要搭乘新幹線或火車，這裡的便當選擇非常豐富。",
                tag: "土產",
                mapLink: "https://www.google.com/maps/search/?api=1&query=Sun+Station+Okayama"
            },
            {
                type: 'shop',
                title: "Don Quijote 岡山站前店",
                subtitle: "24小時挖寶好去處",
                content: "位於車站對面，驚安殿堂唐吉訶德。適合晚上回飯店前補貨，從藥妝、零食到電器應有盡有，價格實惠。",
                tag: "免稅店",
                mapLink: "https://www.google.com/maps/search/?api=1&query=Don+Quijote+Okayama+Station"
            }
        ];
        ...
        document.addEventListener('DOMContentLoaded', () => {
            renderPlan('Day 1');
            renderDaySelector();
            renderGuide('sight'); // Default view
            initWallet();
            initLists();
        ...
        function renderPlan(dayKey) {
            ...
            data.events.forEach(event => {
                ...
                const linkHtml = event.link ? `<a href="${event.link}" target="_blank" class="absolute top-2 right-2 text-stone-300 hover:text-muji-accent"><i class="fas fa-map-marked-alt text-lg"></i></a>` : '';
        ...
        function filterGuide(type) {
            renderGuide(type);
            // Update Tab Style
            const btns = document.querySelectorAll('#guide button');
            btns.forEach(b => {
                b.className = "text-sm text-stone-400 hover:text-muji-text pb-2";
            });
            const activeBtn = document.querySelector(`#guide button[onclick="filterGuide('${type}')"]`);
            if(activeBtn) activeBtn.className = "text-sm font-bold text-muji-text border-b-2 border-muji-accent pb-2";
        }
        ...
        function renderGuide(type) {
            ...
            const filtered = guides.filter(g => g.type === type);
            
            filtered.forEach(guide => {
                const article = document.createElement('article');
                article.className = "bg-white rounded-xl overflow-hidden shadow-sm border border-stone-100";
                
                article.innerHTML = `
                    <div class="p-6">
                        <div class="flex justify-between items-start">
                            <span class="text-xs tracking-widest uppercase text-muji-accent font-bold mb-2 block bg-red-50 px-2 py-1 rounded inline-block">${guide.tag}</span>
                            <a href="${guide.mapLink}" class="bg-stone-100 text-stone-600 text-xs px-3 py-1.5 rounded-full hover:bg-stone-200 transition-colors flex items-center">
                                <i class="fas fa-location-arrow mr-1"></i> 開啟 Google Maps
                            </a>
                        </div>
                        <h3 class="text-xl font-serif font-bold text-muji-text mb-1 mt-2">${guide.title}</h3>
        ...
        async function fetchWeather() {
            ...
            try {
                ...
                if (data.current_weather) {
                    ...
                    document.getElementById('weather-advice').innerText = getOutfitAdvice(temp);
                }
            } catch (error) {
                console.error("Weather fetch failed", error);
                document.getElementById('weather-desc').innerText = "無法取得";
                document.getElementById('weather-advice').innerText = "請檢查網路連線";
            }
        }
        ...
        function getWeatherIcon(code) {
            ...
            if (code <= 3) return '<i class="fas fa-cloud-sun text-stone-400"></i>';
            if (code <= 67) return '<i class="fas fa-cloud-rain text-blue-400"></i>';
            if (code <= 77) return '<i class="fas fa-snowflake text-cyan-300"></i>';
            return '<i class="fas fa-cloud text-stone-400"></i>';
        }

        function getOutfitAdvice(temp) {
            if (temp < 5) return "非常寒冷！請穿著羽絨衣、發熱衣、圍巾，並攜帶暖暖包。";
            if (temp < 10) return "天氣寒冷，建議穿著大衣或厚外套，並搭配層次穿搭。";
            if (temp < 15) return "稍有寒意，建議穿著風衣、毛衣或夾克。";
            if (temp < 22) return "舒適涼爽，穿著長袖上衣或薄外套即可。";
            if (temp < 28) return "溫暖舒適，短袖搭配薄襯衫是很好的選擇。";
            return "天氣炎熱，請穿著透氣衣物並注意防曬補水。";
        }

        // ================= WALLET & LISTS LOGIC (Kept same) =================
        let calcInput = "";
        function initWallet() {
            loadExpenses();
        ...
        function calcAppend(val) { calcInput += val; updateCalcDisplay(); }
        function calcAction(action) { if(action === 'DEL') { calcInput = calcInput.toString().slice(0, -1); updateCalcDisplay(); } }
        function updateCalcDisplay() { document.getElementById('calc-input').value = calcInput; updateCalcPreview(); }
        function updateCalcPreview() {
            try {
                if(!calcInput) { document.getElementById('twd-result').innerText = "0"; return; }
        ...
        function calcResult() {
            try {
                ...
                updateCalcPreview();
            } catch(e) { document.getElementById('calc-input').value = "Error"; calcInput = ""; }
        }
        ...
        async function addExpense() {
            ...
            if(!item || !amount) { alert("請輸入品項與金額"); return; }
            let photoBase64 = null;
            if(fileInput.files[0]) { photoBase64 = await compressImage(fileInput.files[0]); }
            const expense = { id: Date.now(), item, amount, date: new Date().toLocaleDateString(), photo: photoBase64 };
        ...
        function loadExpenses() {
            ...
            expenses.forEach(ex => {
                ...
                let imgHtml = ex.photo ? `<img src="${ex.photo}" class="w-10 h-10 rounded object-cover border mr-3">` : `<div class="w-10 h-10 bg-stone-200 rounded mr-3 flex items-center justify-center text-stone-400"><i class="fas fa-receipt"></i></div>`;
                div.innerHTML = `
                    <div class="flex items-center">
                        ${imgHtml}
                        <div><div class="font-bold text-sm text-muji-text">${ex.item}</div><div class="text-[10px] text-stone-400">${ex.date}</div></div>
                    </div>
                    <div class="text-right"><div class="font-bold text-muji-text">¥${ex.amount}</div><div class="text-[10px] text-stone-400">≈ NT$${Math.round(ex.amount * rate)}</div></div>
                `;
        ...
        function clearExpenses() {
            if(confirm("確定要清空資料嗎？")) { localStorage.removeItem('trip_expenses'); loadExpenses(); }
        }

        function initLists() {
            loadChecklist(); loadMemo();
        ...
        function addChecklist() {
            ...
            input.value = ""; loadChecklist();
        }
        function toggleCheck(id) {
            const list = JSON.parse(localStorage.getItem('trip_checklist') || '[]');
            const item = list.find(i => i.id === id);
            if(item) { item.checked = !item.checked; localStorage.setItem('trip_checklist', JSON.stringify(list)); loadChecklist(); }
        }
        ...
        function saveMemo() { const text = document.getElementById('memo-area').value; localStorage.setItem('trip_memo', text); extractLinks(text); }
        function loadMemo() { const text = localStorage.getItem('trip_memo') || ""; document.getElementById('memo-area').value = text; extractLinks(text); }
